...
// HOMOGENEOUS PATH
      ...
      std::vector<std::vector<int> > paths = g.homogeneous_dfs((int) stoi(commands.at(1)),
                                                                        (int) stoi(commands.at(2)),
                                                                        (int) stoi(commands.at(3)),
                                                                        (int) stoi(commands.at(4)), false,
                                                                        commands.size() == 5 ||
...
// HETEROGENEOUS PATH
      ...
      std::ostringstream oss;
      std::pair<std::vector<std::vector<std::pair<int, int> > >,
          std::vector<std::vector<bool> > > hpaths = g.heterogeneous_dfs((int) stoi(commands.at(1)), // src
                                                                         (int) stoi(commands.at(2)), // dst
                                                                         (int) stoi(
                                                                             commands.at(3)), // discard_rel
                                                                         commands.size() == 5 &&
                                                                         is_true(commands.at(5)), //directed?
                                                                         (int) stoi(commands.at(4))); // length

      std::vector<std::vector<std::pair<int, int> > > &paths = hpaths.first;
      ...
      for (auto it = paths.cbegin(); it != paths.cend(); ++it) {
        oss << "{";
        auto ontol = g.get_ontology((int) stoi(commands.at(1)));
      ...
      std::ostringstream oss;
      std::pair<std::vector<std::vector<std::pair<int, int> > >, std::vector<std::vector<bool> > > hpaths = g.heterogeneous_dfs(
          (int) stoi(commands.at(1)), // src
          (int) stoi(commands.at(2)), // dst
          (int) stoi(commands.at(3)), // discard_rel
      ...
      if (commands.size() > 6 && is_true(commands.at(6))) { // semantic path
        size_t path_id = 0, path_pos = 0;
        for (auto it = paths.cbegin(); it != paths.cend(); ++it) {
          oss << g.get_node_type((int) stoi(commands.at(1))) << "--";
          path_pos = 0;
          for (auto itt = it->cbegin(); itt != it->cend(); ++itt) {
      ...
      return_string = std::to_string(g.get_edge_type_count((int) stoi(commands.at(1))));

// SOURCE ENTITIES
    } else if (commands.at(0) == "in_neighbor") {
      std::ostringstream oss;
      const std::set<int> &neighbors = g.get_in_edges((int) stoi(commands.at(1)));
...
// DEST ENTITIES
      ...
      std::ostringstream oss;
      const std::set<int> &neighbors = g.get_out_edges((int) stoi(commands.at(1)));
      for (auto it = neighbors.cbegin(); it != neighbors.cend(); ++it) {
        oss << *it << ",";
      }
      oss << "\n";
...
// ONTOLOGIES THAT ENTITY BELONGS TO
      ...
      std::vector<int> ontology = g.get_ontology((int) stoi(commands.at(1)));
      std::ostringstream oss;
...
// ENTITIES THAT EXACT MATCH THE INPUT NODE
      ...
      std::ostringstream oss;
      auto siblings = g.get_ontology_siblings((int) stoi(commands.at(1)), 0.0);
...
// ENTITIES SHARING AT LEAST ONE ONTOLOGY WITH GIVEN ENTITY
      ...
      std::vector<std::pair<int, std::set<int> > > siblings =
          g.get_ontology_siblings((int) stoi(commands.at(1)));
      if (commands.size() >= 3 && is_true(commands.at(2))) {
...
// NUMBER OF ENTITIES SHARING AT LEAST ONE ONTOLOGY WITH GIVEN ENTITY
      ...
      std::ostringstream oss;
      std::vector<std::pair<int, int> > res = g.get_ontology_sibling_count(
          (int) stoi(commands.at(1)));
      ...
      std::ostringstream oss;
      std::set<int> res = g.get_neighbor_by_rel((int) stoi(commands.at(1)),
                                                         (int) stoi(commands.at(2)));
...
// ADAMIC ADAR
      ...
      return_string = std::to_string(
          g.adamic_adar((int) stoi(commands.at(1)), (int) stoi(commands.at(2)),
                        commands.size() > 3 ? (int) stoi(commands.at(3)) : 0));

// HETEROGENEOUS ADAMIC ADAR
    } else if (commands.at(0) == "haa") {
      return_string = std::to_string(
          g.heter_adamic_adar((int) stoi(commands.at(1)), (int) stoi(commands.at(2)),
                              (int) stoi(commands.at(3))));

// SEMANTIC PROXIMITY
    } else if (commands.at(0) == "sp") {
      return_string = std::to_string(
          g.semantic_proximity((int) stoi(commands.at(1)), (int) stoi(commands.at(2)),
                               (int) stoi(commands.at(3))));

// MULTI_DIMENSIONAL ADAMIC ADAR
    } else if (commands.at(0) == "maa") {
      return_string = std::to_string(
          g.multidimensional_adamic_adar((int) stoi(commands.at(1)), (int) stoi(commands.at(2)),
                                         (int) stoi(commands.at(3))));

// PERSONALIZED PAGERANK
    } else if (commands.at(0) == "ppr") {
      return_string = std::to_string(
          g.personalized_pagerank((int) stoi(commands.at(1)), (int) stoi(commands.at(2)),
                                  (int) stoi(commands.at(3)), 0.00001,
                                  20, is_true(commands.at(4)), 0.15));

// PREFERENTIAL ATTACHMENT
    } else if (commands.at(0) == "pcrw") {
      std::vector<int> metapath;
      for (int i = 4; i <= stoi(commands.at(3)) + 3; i++) {
        metapath.push_back((int) stoi(commands.at(i)));
      }
      //TODO: add is_directed to CLI argument list
      return_string = std::to_string(
          g.path_constrained_random_walk((int) stoi(commands.at(1)), (int) stoi(commands.at(2)),
                                         metapath, true));
    } else if (commands.at(0) == "pa") {
      return_string = std::to_string(g.preferential_attachment((int) stoi(commands.at(1)),
                                                               (int) stoi(commands.at(2))));

// KATZ
    } else if (commands.at(0) == "katz") {
      return_string = std::to_string(
          g.katz((int) stoi(commands.at(1)), (int) stoi(commands.at(2)),
                 (int) stoi(commands.at(3))));

// CHECK IF TWO NODE CONNECTED BY CERTAIN LINK_TYPE
    } else if (commands.at(0) == "connectedby") {
      std::vector<int> rel_types;
      for (int i = 4; i <= stoi(commands.at(3)) + 3; i++) {
        rel_types.push_back(stoi(commands.at(i)));
      }
      return_string = std::to_string(g.connected_by((int) stoi(commands.at(1)),
                                                    (int) stoi(commands.at(2)),
                                                    rel_types));
    } else if (commands.at(0) == "truelabeled") {
      std::set<std::pair<int, int> > true_labeled_pairs = g.get_entity_pairs_by_triple(
          (int) stoi(commands.at(1)),
          (int) stoi(commands.at(2)),
          (int) stoi(commands.at(3)));
      std::ostringstream oss;
      ...
      std::set<std::pair<int, int> > true_labeled_pairs = g.get_entity_pairs_without_rel(
          (int) stoi(commands.at(1)),
          (int) stoi(commands.at(2)),
          (int) stoi(commands.at(3)));
      std::ostringstream oss;
