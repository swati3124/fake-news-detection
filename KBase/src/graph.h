...
class graph
{
  ...
  std::shared_ptr<nodeMapLoader<node_type> > nodes_ptr;
  std::shared_ptr<relationLoader> edges_ptr;
  std::shared_ptr<edgeTypeLoader<edge_type> > edgetypes_ptr;

  inline void is_node_valid(int id)
  {
  ...
  inline bool is_loop(std::vector<std::pair<int, int> > &path, int id) noexcept {
   ...
   */
  void dfs_helper(int src, int dst, int discard_rel, unsigned max_depth,
                  std::vector<int> &tmp_path, std::set<int> &visited,
                  std::vector<std::vector<int> > &result, bool is_directed, bool depth_only, unsigned depth)
  {
   ...
   */
  void dfs_helper(int src, int dst, int discard_rel, unsigned max_depth,
                  std::vector<std::pair<int, int> > &tmp_path, std::vector<bool> &reverted_rel,
                  std::set<int> &visited,
...
public:
  ...
  graph(nodeMapLoader<node_type> &nodes, relationLoader &edges, edgeTypeLoader<edge_type> &edgetypes) :
    nodes_ptr(&nodes),
    edges_ptr(&edges),
    edgetypes_ptr(&edgetypes) { };

  const std::set<int> &get_out_edges(unsigned src)
  {
    is_node_valid(src);
  ...
  const std::set<int> &get_in_edges(unsigned src)
  {
    is_node_valid(src);
    return edges_ptr->get_edges(src).get_in_neighbors();
  }

  std::vector<std::vector<int> > homogeneous_dfs(int src, int dst, int discard_rel,
      int depth, bool depth_only, bool is_directed)
  {
    is_node_valid(src);
    is_node_valid(dst);
  ...
  std::pair<std::vector<std::vector<std::pair<int, int> > >, std::vector<std::vector<bool> > > heterogeneous_dfs(
    int src, int dst, int discard_rel, bool is_directed, int depth)
  {
    is_node_valid(src);
    is_node_valid(dst);

    std::vector<std::vector<std::pair<int, int> > > path_result;
    std::vector<std::vector<bool> > rel_result;

    try {
      std::vector<std::pair<int, int> > tmp_path;
    ...
    return std::pair<std::vector<std::vector<std::pair<int, int> > >, std::vector<std::vector<bool> > >(
             path_result, rel_result);
  }

  node_type get_node_type(int id)
  {
    return nodes_ptr->getVal(id);
  }

  edge_type get_edge_type(int id)
  {
    return edgetypes_ptr->getVal(id);
  }
   ...
   */
  double adamic_adar(int id1, unsigned id2, int discard_rel = 0)
  {
    is_node_valid(id1);
    is_node_valid(id2);

    std::vector<int> common_neighbors = edges_ptr->get_common_neighbor(id1, id2, discard_rel, false);
   ...
   */
  double semantic_proximity(int src, unsigned dst, int discard_rel = 0)
  {
    std::vector<std::vector<int> > paths = homogeneous_dfs(src, dst, discard_rel, 3, false, false);
   ...
   */
  double multidimensional_adamic_adar(int id1, int id2, int rel_type)
  {
    is_node_valid(id1);
    is_node_valid(id2);
    ...
    double ndc = double(edges_ptr->get_edge_type_count(rel_type)) / edges_ptr->edgeCount;
   ...
   *
   */
  double heter_adamic_adar(int id1, int id2, int rel_type)
  {
    is_node_valid(id1);
    is_node_valid(id2);

    std::vector<std::pair<int, int> > common_neighbors = edges_ptr->get_common_neighbor_except_rel(
    ...
    for (auto it = common_neighbors.cbegin(); it != common_neighbors.cend(); ++it) {
      ...
      if (it->second == edges_ptr->get_type_rel()) {
        size_t degree = edges_ptr->getNodeTypeCount(it->first);
        result += 1.0 / log(degree);
      }
    }
   ...
   */
  double path_constrained_random_walk(int src, int dst, std::vector<int> metapath,
                                      bool is_directed = true)
  {
    ...
    std::vector<bool> is_visited(nodes_ptr->maxKey + 1, false);
    std::vector<double> score(nodes_ptr->maxKey + 1, 0.0);

    std::vector<int> rel_count(nodes_ptr->maxKey + 1, 0);
    ...
    for (auto it = metapath.cbegin(); it != metapath.cend(); ++it) {
      for (int i = 0; i <= nodes_ptr->maxKey; i++) {
   ...
   */
  double personalized_pagerank(int src, int dst, int discard_rel, double delta, int iter,
                               bool is_directed, double damping)
  {
    ...
    std::vector<double> ppr_score(nodes_ptr->maxKey + 1, 0.0);
    std::vector<double> old_score(nodes_ptr->maxKey + 1, 0.0);
    ...
    while (cnt < iter) { // run at most *iter* iterations
      ...
      for (int i = 0; i <= nodes_ptr->maxKey; i++) {
        ...
        const std::set<std::pair<int, int> > &in_neighbors = edges_ptr->get_edges(i).get_backward(); // all in-coming edges
        ...
        if (!is_directed) {
          const std::set<std::pair<int, int> > &out_neighbors = edges_ptr->get_edges(i).get_forward(); // all out-going edges
   ...
   *
   */
  double preferential_attachment(int id1, int id2)
  {
    is_node_valid(id1);
    is_node_valid(id2);
   ...
   */
  double katz(int id1, int id2, int discard_rel, int max_length = 3,
              double beta = 0.05)
  {
    is_node_valid(id1);
    is_node_valid(id2);
    ...
    for (int i = 3; i <=
      ...
      std::vector<std::vector<int> > paths = homogeneous_dfs(id1, id2, discard_rel, max_length, true, false);
  ...
  inline std::vector<int> get_ontology(int id)
  {
    is_node_valid(id);
    return edges_ptr->get_ontology(id);
  }

  inline std::vector<std::pair<int, std::set<int> > > get_ontology_siblings(int id)
  {
    is_node_valid(id);
    return edges_ptr->get_ontology_siblings(id);
  }

  inline std::set<int> get_ontology_siblings(int id, double tol)
  {
    is_node_valid(id);
    return edges_ptr->get_ontology_siblings(id, tol);
  }

  inline std::vector<std::pair<int, int> > get_ontology_sibling_count(int id)
  {
    is_node_valid(id);
    return edges_ptr->get_ontology_sibling_count(id);
  }

  bool connected_by_helper(int src, int dst, int pos, std::vector<int> &link_type,
                           bool is_directed)
  {
    edge_list &edges = edges_ptr->get_edges(src);
  ...
  bool connected_by(int src, int dst, std::vector<int> link_type, bool is_directed = false)
  {
    is_node_valid(src);
    is_node_valid(dst);
  ...
  bool connected_by(int src, int dst, int link_type, bool is_directed = false)
  {
    is_node_valid(src);
    is_node_valid(dst);
  ...
  std::vector<std::pair<int, int> > get_entity_pairs_by_rel(int rel_type,
      double sample_rate = 0.1)
  {
    ...
    for (int i = nodes_ptr->getMinKey(); i <= nodes_ptr->maxKey; i++) {
      edge_list &edges = edges_ptr->get_edges(i);
      for (auto it = edges.get_forward().cbegin(); it != edges.get_forward().cend(); ++it) {
        if (it->second == rel_type && ((double) rand() / RAND_MAX <= sample_rate)) {
          res.push_back(std::pair<int, int>(i, it->first));
        }
      }
    }
  ...
  inline std::set<std::pair<int, int> > get_entity_pairs_by_triple(int src, int dst,
      int rel_type,
      double sample_rate = 0.1)
  {
  ...
  inline std::set<std::pair<int, int> > get_entity_pairs_without_rel(int src,
      int dst,
      int rel_type,
      double sample_rate = 0.1)
  {
  ...
  std::set<std::pair<int, int> > get_entity_pairs_by_triple_helper(int src, int dst,
      int rel_type,
      double sample_rate = 0.1,
      bool exclude_rel = false)
  {
    ...
    std::set<int> src_set = get_ontology_siblings(src, 0.0);
    std::set<int> dst_set = get_ontology_siblings(dst, 0.0);

    // Step 2: Get true labeled node pairs from previous sets

    std::set<std::pair<int, int> > entity_pairs;
    std::set<std::pair<int, int> > entity_pairs_with_rel;
      ...
      for (auto p = edges.cbegin(); p != edges.cend(); ++p) {
        if (!exclude_rel) { // get all entity pairs with rel_type
          if (p->second == rel_type && dst_set.find(p->first) != dst_set.end()) { // rel match and dst is in the set
            entity_pairs_with_rel.insert(std::pair<int, int>(*it, p->first));
          }
        } else { // get all entity pairs without rel_type
          if (p->second != rel_type && dst_set.find(p->first) != dst_set.end()) {
            entity_pairs.insert(std::pair<int, int>(*it, p->first));
          }
        }
      }
    }
      ...
      for (auto p = edges.cbegin(); p != edges.cend(); ++p) {
        if (!exclude_rel) {
          if (p->second == rel_type && src_set.find(p->first) != src_set.end()) { // rel match and src is in the set
            entity_pairs_with_rel.insert(std::pair<int, int>(p->first, *it));
          }
          ...
          if (p->second != rel_type && src_set.find(p->first) != src_set.end()) {
            entity_pairs.insert(std::pair<int, int>(p->first, *it));
          }
        }
  ...
  inline int get_edge_type_count(int rel_type)
  {
    return edges_ptr->get_edge_type_count(rel_type);
  }

  std::set<int> get_neighbor_by_rel(int src, int rel_type, bool is_directed = false)
  {
    is_node_valid(src);
